...
class TestInjection extends FlatSpec with ShouldMatchers {
  ...
  object BlankProject extends Template with Create {
    def name = "blank"
    def description = ""
    def files = TemplateFile(TestHelper.makeProperPath(List("lifty-engine","src","test","resources","projectblank.txt")),
      TestHelper.makeProperPath(List("lifty-engine","src","test","output","projectblank.txt"))) :: Nil
    def arguments = Nil
  }
  ...
  object BasicProject extends Template with Create {
    ...
    injectContentsOfFile(TestHelper.makeProperPath(List("lifty-engine","src","test","resources","basic_inject_Blank_Point.txt"))).into("projectblank.txt").at("Point")
  }
  ...
  **/
  ...
  "When running Model there" should "be no sign of the injection points" in {
    ...
    val wantedResult = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","resources","correct_results","_temp_model_running_model.txt")))
    val wantedResultIs = new FileInputStream(wantedResult)
    val wantedResultS = Source.fromInputStream(wantedResultIs)
