...
class TestTemplateCreating extends FlatSpec with ShouldMatchers {
  ...
  "Snippet template" should "be created" in {
    ...
    val f1 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","snippet.scala")))
  ...
  it should "create it's dependencies" in {
    ...
    val f1 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","snippet.scala")))
    val f2 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","dependent.scala")))
    val f3 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","index.html")))
    f1.exists should be === true
 ...
 "create snippet with index with empty mypackage" should "create both templates" in  {
   ...
   val f1 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","snippet.scala")))
   val f2 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","empty.html")))
   val f3 = new File(TestHelper.makeProperPath(List("lifty-engine","src","test","output","index.html")))
   f1.exists should be === true
   f2.exists should be === true
   f3.exists should be === true
